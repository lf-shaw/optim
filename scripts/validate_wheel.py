...
def validate(path: Path) -> None:
    ...
    with ZipFile(path) as archive:
        ...
        library = tomllib.loads(
            archive.read("optim/LIBRARY.toml").decode("utf-8")
        )
        metadata_name = next(
    ...
    wheel_version = None if wheel_version_match is None else wheel_version_match.group(1)
    ...
    core_python = {
        name
        for name in names
        if name.startswith("optim/_core/") and name.endswith(".py")
    }
    unexpected_python = sorted(core_python - ALLOWED_CORE_PYTHON)
    if unexpected_python:
        raise SystemExit(f"wheel 暴露了 core Python 实现: {unexpected_python}")

    forbidden = sorted(
        name
        for name in names
        if name.startswith("optim/_core/")
        and PurePosixPath(name).suffix in {".pyi", ".c", ".cpp", ".html"}
    )
    if forbidden:
        raise SystemExit(f"wheel 包含禁止的 core 中间文件: {forbidden}")
    ...
    missing_modules = sorted(
        ...
        if not any(
            name.startswith(f"optim/_core/{module}.") for name in compiled
        )
    )
    if missing_modules:
    ...
    print(
        f"✅ wheel 校验通过：{len(compiled)} 个 core 扩展，"
